...
import collections
global udpscan_dic,recent_udpscans,recent_scans,halfscandic, fullscandic, srcip_addr, horizontal_scan, scanned_ports, vertical_scan, scanned_ips,scanned_udpips,scanned_udpports, threewayhandshake, fullscancheck,cidr_dic
...
recent_udpscans = collections.OrderedDict()
...
def halfconnect_scan(ts, buf, src_ip, dst_ip, src_port, dst_port, seq_num, ack_num, syn_flag, rst_flag, ack_flag):
    ...
    if syn_flag == 1 and ack_flag == 0:
        ...
        if tempkey not in recent_scans:
            t1 = datetime.strptime(date_time, fmt)
            if recent_scans:
                for k,v in recent_scans.iteritems():
                    t2 = datetime.strptime(v, fmt)
        ...
        if halfscandic.has_key(dstip_srcip + "_" + str(ack_num - 1)):
            ...
            if ip_classc in cidr_dic:
                asn_cidr = cidr_dic[ip_classc]
            else:
                obj = IPWhois(dst_ip)
                res = obj.lookup_whois()
                asn_cidr = res["asn"]
                cidr_dic[ip_classc] = asn_cidr

            # ip_ts --- distinct keys for vertical_scan and horizontal scan dictionaries
            if asn_cidr == 'NA':
                hor_ip_ts = a + "." + b + "." + c + '_' + str(src_port)+ '_' + date
                ver_ip_ts = a + "." + b + "." + c + '_' + src_ip + '_' + date
            else:
                hor_ip_ts = asn_cidr + '_' + str(src_port)+ '_' + date
                ver_ip_ts = asn_cidr + '_' + src_ip+ '_' + date
            #print hor_ip_ts
        ...
        if halfscandic.has_key(srcip_dstip + "_" + str(seq_num - 1)):
            ...
            if asn_cidr == 'NA':
                hor_ip_ts = a + "." + b + "." + c + '_' + str(dst_port)+ '_' + date
                ver_ip_ts = a + "." + b + "." + c + '_' + dst_ip+ '_' + date
            else:
                hor_ip_ts = asn_cidr + '_' + str(dst_port)+ '_' + date
                ver_ip_ts = asn_cidr + '_' + dst_ip+ '_' + date
...
def udpscan_check(ip_hdr,ts,buf):
    ...
    if isinstance(ip_hdr.data, dpkt.udp.UDP):
        ...
        src_port = udp.sport
        dst_port = udp.dport
        asn_cidr = find_isp_source_dest(src_ip)
        hor_ip_ts = find_hor_key_src_dest(asn_cidr,dst_port,date)
        ver_ip_ts = find_ver_key_src_dest(asn_cidr,dst_ip,date)
        ...
        if temp in udpscan_dic:
            ...
            if udpscan_dic[revtemp]['closed'] == 0:
                udpscan_dic[revtemp]['closed'] = 3
            asn_cidr = find_isp_dest_source(dst_ip)
            hor_ip_ts = find_hor_key_dest_src(asn_cidr,src_port,date)
            ver_ip_ts = find_ver_key_dest_src(asn_cidr,src_ip,date)
        ...
        if code == 3 and types == 3:
                ...
                if temp in udpscan_dic:
                    ...
                    horizontal_scan2(hor_ip_ts,date_time,src_ip)
                    vertical_scan1(ver_ip_ts,date_time,dst_port)
def find_isp_source_dest(src_ip):
    a, b, c, d = src_ip.split(".")
    ip_classc = a + "." + b + "." + c
    if ip_classc in cidr_dic:
        asn_cidr = cidr_dic[ip_classc]
        ...
        cidr_dic[ip_classc] = asn_cidr
    return asn_cidr
def find_isp_dest_source(dst_ip):
    a, b, c, d = dst_ip.split(".")
    ip_classc = a + "." + b + "." + c
    if ip_classc in cidr_dic:
        asn_cidr = cidr_dic[ip_classc]
        ...
        cidr_dic[ip_classc] = asn_cidr
    return asn_cidr
def find_hor_key_src_dest(asn_cidr,dst_port,date):
    if asn_cidr == 'NA':
        hor_ip_ts = a + "." + b + "." + c + '_' + str(dst_port)+ '_' + date
    else:
        hor_ip_ts = asn_cidr + '_' + str(dst_port)+ '_' + date
    return hor_ip_ts
def find_hor_key_dest_src(asn_cidr,src_port,date):
    if asn_cidr == 'NA':
        hor_ip_ts = a + "." + b + "." + c + '_' + str(src_port)+ '_' + date
    else:
        hor_ip_ts = asn_cidr + '_' + str(src_port)+ '_' + date
    return hor_ip_ts
def find_ver_key_src_dest(asn_cidr,dst_ip,date):
    if asn_cidr == 'NA':
        ver_ip_ts = a + "." + b + "." + c + '_' + dst_ip+ '_' + date
    else:
        ver_ip_ts = asn_cidr + '_' + dst_ip+ '_' + date
    return ver_ip_ts
def find_ver_key_dest_src(asn_cidr,src_ip,date):
    if asn_cidr == 'NA':
        ver_ip_ts = a + "." + b + "." + c + '_' + src_ip + '_' + date
    else:
        ver_ip_ts = asn_cidr + '_' + src_ip+ '_' + date
...
def check_recent_scans(temp,date_time):
    t1 = datetime.strptime(date_time, fmt)
    if temp not in recent_udpscans:
        if recent_udpscans:
            for k,v in recent_udpscans.iteritems():
                t2 = datetime.strptime(v, fmt)
                if (t1 - t2).seconds <900:
                    break
                else:
                    del recent_udpscans[k]
        print 'not in recent_udpscans'
        recent_udpscans[temp]= date_time
        return 1
    else:
        if recent_udpscans:
            for k,v in recent_udpscans.iteritems():
...
def main():
    ...
    for ts, buf in pcap:
        try:
           ...
           print e
    f = open("horizontal_isp.txt", "wb")
    f1 = open("vertical_isp.txt", "wb")
